...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			Dot dot = new Dot(x,y);
			if (tree == null) { // if there is nothing then a new tree is drawn
			...
			// TODO: YOUR CODE HERE
			found = tree.findInCircle((double)mouseX,(double)mouseY,(double)mouseRadius);
		}
		else {
	...
	private void test2() {
		...
		int sizeCount = 0;
		int errorsFound = 0;
		tree = new PointQuadtree<Dot>(new Dot(350,350), 0,0,800,600); // creates 10 different points
		tree.insert(new Dot(450,150)); 
		tree.insert(new Dot(550,250)); 
		tree.insert(new Dot(200,450)); 
		tree.insert(new Dot(250,200)); 
		tree.insert(new Dot(125,500)); 
		tree.insert(new Dot(250,475)); 
		tree.insert(new Dot(225,525)); 
		tree.insert(new Dot(215,490)); 
		tree.insert(new Dot(410,310)); 
		
		sizeCount = tree.size();
		if (sizeCount != 10) { // If not expected size then an error
			errorsFound = errorsFound + 1;
		}
		// uses test find for two points
		errorsFound += testFind(200,200,50,4,2,1);
		errorsFound += testFind(350,350,50,8,5,1);
		if (errorsFound==0) System.out.println("test 2 passed!");
	}
	/**
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		//if a quadrant has a child, call draw tree on the child, g, new sub quadtree, with one level higher
		if (tree.hasChild(1)) {
